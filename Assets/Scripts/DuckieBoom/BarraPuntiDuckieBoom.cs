...
public class BarraPuntiDuckieBoom : MonoBehaviour {
	...
	void Vittoria()
    {
        var clones = GameObject.FindGameObjectsWithTag("Duckie");
        foreach (var duckie in clones)
        {
            Destroy(duckie);
        }
		foreach (var d in Disable)
        {
            d.SetActive(false);
        }
        WinCanv.alpha = 1;
